...
const ManagerLeaveSummary = () => {
  ...
  const fetchEmployees = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        "https://lms-be-beta.vercel.app/api/User",
        {
          params: { department },
        }
      );
  ...
  const getLeaveData = useCallback(async () => {
    try {
      const result = await axios.get(
        "https://lms-be-beta.vercel.app/api/LeaveView"
      );
  ...
  const downloadPdf = (username) => {
    ...
    doc.save(
      `${username}-leave-summary-${new Date().toLocaleDateString()}.pdf`
    );
  };
                ...
                Object.keys(leaveSummary[username]).map((leaveType, i) => (
                  <Fragment key={`${username}-${i}`}>
                    <tr>
                      {i === 0 && (
                        <td
                          rowSpan={Object.keys(leaveSummary[username]).length}
                        >
                          {username}
                        </td>
                      )}
